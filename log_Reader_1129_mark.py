...
def call_keyword_dict(list_name):
    ...
    if list_name in ['QT0','QT1']:
        print('QT')
        keyword_station =   {'r':['\[CmdResult:\] (FAIL)','\[Test Fail\](.*)','\[ERROR\](.*)']
        ...
        print('ATLAS')
        keyword_station =   {'r':['(Failed command|TestFailedError)']
                            ,'g':['Test finished event for (.*)','Successfully completed command ([\w ]*)','Logging data for ([\w ]*)']
    ...
    # elif list_name in ['ColorCal']:
    #     print('ColorCal')
    elif list_name in ['FCT']:
        print('FCT')
    elif list_name in ['FACT']:
        print('FACT')
    elif list_name in ['CBT']:
        print('CBT')
...
# Pass/Fail功能待建立 (部分fail log不在测试中发生)
...
# ['ALS','ACTUATION','BUTTON','FORCE'], 没什么问题, (Fail的测试项漏切几行log, 这周调整)
